...
def main(args):
    ...
    # Create a random forest on the trainining data.
    #
    ...
    generator = np.random.RandomState(args.seed)
    random_forest = RandomForest(trees=args.trees, random_generator=generator)
    random_forest.fit(train_data, train_target, criterion='entropy', max_depth=args.max_depth,
                      feature_subsampling=args.feature_subsampling, bootstrapping=args.bootstrapping)

    test_prediction = random_forest.predict(test_data)
    train_prediction = random_forest.predict(train_data)
